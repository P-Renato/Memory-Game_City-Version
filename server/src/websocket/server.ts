...
async function handleWebSocketMessage(ws: WebSocketClient, message: WebSocketMessage) {
  ...
  switch (type) {
    case 'JOIN_ROOM':
     ...
     case 'FLIP_CARD':  // ADD THIS CASE STATEMENT - IT WAS MISSING!
      if (!ws.roomId || !ws.userId) {
        console.error('❌ FLIP_CARD: Missing roomId or userId');
        return;
      }
      
      if (data?.cardIndex === undefined) {
        console.error('❌ FLIP_CARD: Missing cardIndex');
        return;
      }
      
      console.log(`🎮 ${ws.username} flipping card ${data.cardIndex} in room ${ws.roomId}`);
      ...
      try {
        ...
        console.log(`✅ Card flip processed: ${result.matchResult}`);
        ...
        if (result.matchResult === 'NO_MATCH') {
          ...
          setTimeout(async () => {
            try {
              ...
              if (turnSwitchedRoom) {
                console.log(`🔄 Switching turn from ${ws.userId} to ${turnSwitchedRoom.gameState.currentTurn}`);
        ...
        if (result.matchResult === 'MATCH') {
          console.log('✅ Match found! Broadcasting match notification');
